...
SYSTEM_PROMPT = (
    ...
    "- 'What's trending?' → get_trending_topics() first; if it returns empty results, immediately follow up with search_news + search_reddit to find what's trending.\n"
    "- 'Predict trends / what will trend / news trends this week' → search_news(recent news) + search_web(trending topics this week) + search_reddit(hot discussions) — do NOT use get_trending_topics alone for prediction questions.\n"
    ...
    "- CRITICAL: If a first search returns weak or empty results, immediately call another tool (different query or different platform). Never return an empty or 'no results' answer without trying at least 2 tools.\n"
